...
// =====================================================
// RESUME A QUESTION'S TIMER AFTER A HOST PAUSE
// =====================================================
// FIX #3: when the host reconnects mid-question, the original
// questionEndsAt timestamp has already been "used up" by real time
// passing while paused. We extend it by exactly how long the pause
// lasted, then reschedule the timer for the remaining duration, and
// tell clients the new deadline so their countdowns resync too.
const resumeQuestionTimer = (io, roomCode) => {
  const room = getRoom(roomCode);
  if (!room) return;
  if (room.currentQuestionIndex < 0 || !room.questionEndsAt) return;
  if (!room.pausedAt) return; // nothing to resume from

  const pauseDuration = Date.now() - room.pausedAt;
  room.questionEndsAt += pauseDuration;
  room.pausedAt = null;

  const remaining = room.questionEndsAt - Date.now();

  io.to(roomCode).emit("question-resumed", {
    questionEndsAt: room.questionEndsAt,
  });
  ...
  if (remaining <= 0) {
    // Deadline already passed even after extending — just close it out
    endQuestion(io, roomCode);
    return;
  }
...
const endQuiz = async (io, roomCode) => {
  ...
  // =====================================================
  // SAVE FINISHED STATUS TO MONGODB (best-effort — never blocks
  // telling players the game is over, even if this fails)
  // =====================================================

  try {
    const liveRoom = await LiveRoom.findOne({ roomCode });

    if (!liveRoom) {
      console.error("LiveRoom not found:", roomCode);
    } else {
      liveRoom.status = "FINISHED";
      liveRoom.endedAt = new Date();
      liveRoom.participants = participants;
      liveRoom.questionStats = questionStats;

      await liveRoom.save();

      console.log("ROOM SAVED SUCCESSFULLY:", liveRoom.roomCode, liveRoom.status);
    }
  } catch (error) {
    console.error("FAILED TO SAVE FINISHED ROOM:", error);
    // FIX #1: intentionally NOT returning here — a DB save failure
    // must not prevent players from being told the quiz is over.
  }
          ...
          ? {
              ...
              accuracy:
                quiz.questions.length > 0
                  ? Math.round(
                      (stats.correctAnswers / quiz.questions.length) * 100
                    )
                  : 0,
            }
          : null,
  ...
  if (host?.socketId) {
    io.to(host.socketId).emit("quiz-finished", { leaderboard });
  }
};
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", ({ roomCode, name, userId, isHost }) => {
      ...
      const existingPlayer = room.players.find(
        (p) => String(p.userId) === normalizedUserId
      );
      ...
      if (room.status === "IN_PROGRESS" && !existingPlayer) {
        socket.emit("room-error", {
          message: "This quiz is already in progress. You can't join mid-game.",
        });
      ...
      if (existingPlayer) {
        ...
        // Make sure the actual host is marked as host
        ...
        room.players.push({
          ...
          name,
          isHost: !!isHost && normalizedHostId === normalizedUserId,
          score: 0,
          answered: false,
          connected: true,
        });
      ...
      // Host reconnect — resume the room AND its stuck timer
      if (normalizedHostId === normalizedUserId && room.status === "PAUSED") {
        room.status = "IN_PROGRESS";

        io.to(roomCode).emit("host-reconnected");

        resumeQuestionTimer(io, roomCode);
      }
    });
    ...
    socket.on("get-room-state", async ({ roomCode }) => {
      ...
      // FIX #2: always respond, even when we can't produce a question,
      // so the client never hangs waiting silently.
      if (!quiz) {
        socket.emit("room-state", { status: room.status, question: null });
        return;
      }
    ...
    socket.on("submit-answer", async ({ roomCode, questionIndex, selectedAnswer }) => {
      ...
      // Don't accept answers while the room is paused (host disconnected)
    ...
    socket.on("next-question", async ({ roomCode }) => {
      ...
      if (String(room.hostId) !== String(socket.data.userId)) {
        socket.emit("room-error", {
          message: "Only the host can move to the next question",
        });
      ...
      if (room.status !== "IN_PROGRESS") {
        socket.emit("room-error", {
          message: "The quiz is not currently in progress",
        });
    ...
    socket.on("disconnect", () => {
      ...
      // Remove players who disconnect while waiting
      ...
      if (player?.isHost && room.status === "IN_PROGRESS") {
        room.status = "PAUSED";
        room.pausedAt = Date.now(); // FIX #3: remember when we paused
        clearRoomTimer(roomCode);
